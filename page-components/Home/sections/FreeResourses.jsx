...
const FreeResourcesPageTest = ({ onPageChange, onQuizNavigation, onAuthAction }) => {
    ...
    const handleCardClick = (item) => {
        ...
        if (!courseId && !item?.entityType) {
            ...
            if (item?.setting?.restrictFreeContent === true && !enrolledCourses.has(item?.id)) {
                ...
                const studentData = typeof window !== 'undefined' ? localStorage.getItem('studentData') : null;
    ...
    const handleLoginWarningConfirm = () => {
        ...
        if (!authToken && !isAuthenticated) {
            router.push('/login', { state: { data: "freeResource" } })
        }
    };
...
export default FreeResourcesPageTest
