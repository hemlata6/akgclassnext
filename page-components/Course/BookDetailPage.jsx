...
import { Icons, LAYOUT_PADDING, BRAND_GREEN_CLASS, BRAND_GREEN_HOVER_CLASS, TEXT_GREEN } from '../../constants/Icons';
...
const BookDetailPage = ({ bookData, onBack }) => {
    ...
    const handleLogoutError = (errorCode, errorDescription) => {
        if (LOGOUT_ERROR_CODES.has(Number(errorCode))) {
            localStorage.removeItem('authToken');
            localStorage.removeItem('user');
            setShowLoginModal(true);
            setShowMobilePaymentModal(false);
            setShowCheckoutFormModal(false);
            const msg = errorDescription || 'Session expired. Please login again.';
    ...
    const handleProceedToCheckoutDirect = () => {
        if (!pricing) return;
    ...
    useEffect(() => {
        if (localStorage.getItem('authToken') && checkoutCartItem && !showLoginModal) {
            handleProceedToCheckoutDirect();
            setCheckoutCartItem(null);
        }
    }, [localStorage.getItem('authToken'), showLoginModal, checkoutCartItem]);
    ...
    const handleProceedToCheckout = async (customerDetails = checkoutForm) => {
        if (!pricing?.id || !bookData?.id) {
            const errorMsg = 'Please select a valid book before checkout.';
            setErrorBarMessage(errorMsg);
            setShowErrorBar(true);
            return;
        }
        ...
        const checkoutToken = tokenFromUrl || urlToken || localStorage.getItem('authToken');
                            ...
                            <div className="flex flex-col lg:h-full">
                                ...
                                <div className="mt-3 space-y-1.5">
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-2 rounded-lg font-bold text-xs transition-all active:scale-95 flex items-center justify-center gap-1.5 ${!pricing || isProcessing
                                ...
                                <div className="space-y-4">
                                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-2">
                                        ...
                                        <p className="text-2xl font-bold text-slate-900">
                                            ₹{discountedPrice?.toLocaleString('en-IN') || 0}
                                        </p>
