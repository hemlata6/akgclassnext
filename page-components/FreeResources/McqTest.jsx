...
const MCQTest = () => {
  ...
  // Load quiz data from sessionStorage or localStorage
  const [quizData, setQuizData] = useState(() => {
    // Check if running in browser
    if (typeof window === 'undefined') return null;

    // Try to get from sessionStorage first (for navigation state)
    const sessionData = sessionStorage.getItem('currentQuizData');
    if (sessionData) {
  ...
  // No longer needed - data is loaded in useState initializer
  // useEffect(() => {
  //   if (router.query?.quizData) {
  //     setQuizData(JSON.parse(router.query.quizData));
  //     localStorage.setItem('quizData', router.query.quizData);
  //   }
  // }, [router.query?.quizData]);
  ...
  const handleConfirmExit = async () => {
    // Clear quiz data from localStorage
    if (typeof window !== 'undefined') localStorage.removeItem('currentQuizData');
          ...
          <div className="bg-white rounded-xl shadow-lg p-8">
            ...
            <div className="text-center">
              <button
                ...
                className="text-white font-bold py-4 px-8 rounded-lg text-lg transition-colors duration-200 flex items-center mx-auto"
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 md:space-x-6">
              <div className={`flex items-center px-3 sm:px-4 py-2 rounded-lg font-semibold text-sm sm:text-base ${timeRemaining <= 300 ? 'bg-red-100 text-red-700' : 'text-white'
                }`}
                style={timeRemaining > 300 ? { backgroundColor: BRAND_GREEN } : {}}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <select
                        ...
                        className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 text-sm"
                        style={{ '--tw-ring-color': BRAND_GREEN, borderColor: 'rgb(209, 213, 219)' }}
                        onFocus={(e) => { e.target.style.borderColor = BRAND_GREEN; e.target.style.outlineColor = BRAND_GREEN; }}
              ...
              <div className="flex flex-col space-y-4">
                ...
                <div className="flex justify-center sm:justify-end">
                  <button
                    ...
                    className="flex items-center px-6 py-2 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200 text-sm sm:text-base min-w-[120px] justify-center"
                    style={{ backgroundColor: BRAND_GREEN }}
