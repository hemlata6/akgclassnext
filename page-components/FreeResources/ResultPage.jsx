...
import { BRAND_GREEN, BRAND_GREEN_HOVER } from '../../constants/Icons';
...
const Result = () => {
    ...
    const [quizData, setQuizData] = useState(() => {
        ...
        const savedQuizData = sessionStorage.getItem('quizData') || localStorage.getItem('quizData');
        if (savedQuizData) {
            try {
            ...
            if (!testResults && response) {
                ...
                if (response.analysis && Array.isArray(response.analysis)) {
                    response.analysis.forEach(section => {
                        const sectionPercentage = Math.round(section.percent || 0);
                ...
                const overallPercentage = Math.round(response.percent || 0);
    ...
    const performanceData = [
        {
            ...
            percentage: testResults?.totalQuestions > 0 ? Math.round(((testResults?.correct || 0) / testResults?.totalQuestions) * 100) : 0,
            color: 'bg-green-500',
            icon: CheckCircle,
        ...
        },
        {
            ...
            percentage: testResults?.totalQuestions > 0 ? Math.round(((testResults?.wrong || 0) / testResults?.totalQuestions) * 100) : 0,
            color: 'bg-red-500',
            icon: XCircle,
        ...
        },
        {
            ...
            percentage: testResults?.totalQuestions > 0 ? Math.round(((testResults?.skipped || 0) / testResults?.totalQuestions) * 100) : 0,
            color: 'bg-yellow-500',
            icon: Clock,
        ...
        },
        {
            ...
            percentage: testResults?.totalQuestions > 0 ? Math.round(((testResults?.notAttempted || 0) / testResults?.totalQuestions) * 100) : 0,
            color: 'bg-gray-500',
            icon: BookOpen,
                            ...
                            <div className={`grid gap-6 sm:gap-8 ${testResults?.sectionWiseResults && Object.keys(testResults?.sectionWiseResults).length > 1
                                ...
                                <div className="space-y-4 sm:space-y-6">
                                    ...
                                    <div className={`grid gap-3 sm:gap-4 ${testResults?.sectionWiseResults && Object.keys(testResults?.sectionWiseResults).length > 1
                                        ...
                                        <div className="rounded-lg p-3 sm:p-4" style={{ backgroundColor: `${BRAND_GREEN}10` }}>
                                            <div className="text-xl sm:text-2xl font-bold" style={{ color: BRAND_GREEN }}>{testResults?.totalQuestions}</div>
                                            <div className="text-xs sm:text-sm" style={{ color: BRAND_GREEN }}>Total Questions</div>
                                        </div>
          ...
          <div className="mt-6 sm:mt-8 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-xl p-6 sm:p-8 text-center text-white">
              ...
              Download Certificate
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4 sm:px-0">
                    <button
                        ...
                        className="text-white font-bold py-3 px-6 sm:px-8 rounded-lg transition-colors duration-200 flex items-center justify-center disabled:opacity-50 text-sm sm:text-base w-full sm:w-auto"
                        style={{ backgroundColor: BRAND_GREEN }}
                        onMouseEnter={(e) => !e.currentTarget.disabled && (e.currentTarget.style.backgroundColor = BRAND_GREEN_HOVER)}
            ...
            >
              ...
              Retake Test
                    ...
                    <button
                        onClick={() => router.push('/')}
                        className="text-white font-bold py-3 px-6 sm:px-8 rounded-lg transition-colors duration-200 text-sm sm:text-base w-full sm:w-auto"
                        style={{ backgroundColor: BRAND_GREEN }}
                        onMouseEnter={(e) => e.target.style.backgroundColor = BRAND_GREEN_HOVER}
                        onMouseLeave={(e) => e.target.style.backgroundColor = BRAND_GREEN}
                    >
                        Go to Home
                    </button>
                </div>
                ...
                {showSolutionsModal && solutions.length > 0 && (
                    <div className="fixed inset-0 z-50 bg-white overflow-y-auto">
                        {/* Header */}
                        <div className="sticky top-0 text-white shadow-lg z-10" style={{ backgroundColor: BRAND_GREEN }}>
                            <div className="mx-auto px-4 sm:px-6 py-3 sm:py-4">
                                ...
                                {testResults?.sections && testResults?.sections.length > 1 && (
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                        <label className="text-white text-sm font-medium whitespace-nowrap">Filter by Section:</label>
                                        <select
                                            value={selectedSectionFilter}
                                            onChange={(e) => setSelectedSectionFilter(e.target.value)}
                                            className="px-3 py-2 border rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-white min-w-0 flex-1 sm:flex-initial sm:min-w-[200px]"
                                            style={{ backgroundColor: BRAND_GREEN_HOVER, borderColor: 'rgba(255,255,255,0.3)' }}
                                        >
