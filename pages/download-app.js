...
const DownloadAppPage = () => {
    ...
    const downloadLinks = [
        {
            ...
            url: 'https://play.google.com/store/apps/details?id=com.classiolabs.nextgenca',
            icon: (
        ...
        },
        {
            ...
            url: 'https://apps.apple.com/in/app/nextgen-ca/id6759248050',
            icon: (
        ...
        },
        {
            ...
            url: 'https://apps.microsoft.com/detail/9PG3KXKV4XKM?hl=en-us&gl=IN&ocid=pdpshare',
            icon: (
                <svg width="80" height="80" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0 3.449L9.75 2.1v9.451H0z" fill="#F25022" />
                    <path d="M10.949 2.1L24 0v11.4H10.949z" fill="#7FBA00" />
                    <path d="M0 12.6h9.75V22.05L0 20.699z" fill="#0d5a3e" />
    ...
    return (
        <>
            <Header cartCount={0} />
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50 to-green-50 py-12 md:py-24">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-wrap justify-center gap-6 mb-16">
                        {downloadLinks.map((link) => (
                            <div
                                key={link.id}
                                className={`w-full max-w-xs bg-white rounded-2xl border-2 border-slate-200 cursor-pointer transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl hover:border-emerald-300 overflow-hidden group`}
                                onMouseEnter={() => setHoveredCard(link.id)}
                                onMouseLeave={() => setHoveredCard(null)}
                                onClick={() => handleDownload(link.url)}
                            >
                                {/* Gradient Background */}
                                <div className={`h-1.5 bg-gradient-to-r from-emerald-900 to-green-700`}></div>
                                ...
                                <div className="p-8 flex flex-col items-center text-center h-full">
                                    ...
                                    {/* Platform Badge */}
                                    <span className={`inline-block mb-4 text-xs font-semibold px-3 py-1 rounded-full ${primaryClass} bg-opacity-10 text-emerald-900`}>
                                        {link.platform}
                                    </span>
                                    ...
                                    <button
                                        onClick={() => handleDownload(link.url)}
                                        className={`w-full bg-gradient-to-r from-emerald-900 to-green-700 text-white px-6 py-3 rounded-xl font-bold text-sm transition-all duration-300 hover:shadow-lg hover:from-emerald-800 hover:to-green-600 active:scale-95 transform hover:scale-105`}
                                    >
