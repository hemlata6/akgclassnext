import { Box, Button, Card, Checkbox, Chip, Dialog, DialogActions, DialogContent, FormControl, FormHelperText, Grid, IconButton, InputAdornment, InputLabel, ListItemText, MenuItem, OutlinedInput, Select, Stack, Step, StepLabel, Stepper, StepConnector, styled, TextField, Typography, useMediaQuery, Fade, CardContent, CardMedia, CardActions, Container } from '@mui/material';
...
const ModernChip = styled(Chip)(({ chipcolor = 'primary' }) => ({
    ...
    color: chipcolor === 'primary' ? modernColors.primary.main : modernColors.accent.main,
    fontWeight: 600,
    fontSize: '0.75rem',
    height: '28px',
    margin: '2px',
    border: `1px solid ${chipcolor === 'primary' ? modernColors.primary.main : modernColors.accent.main}30`,
}));

const ModernCheckoutButton = styled(Button)(({ theme }) => ({
    background: modernColors.primary.gradient,
    color: 'white',
    fontWeight: 'bold',
    fontSize: '14px',
    ...
    '&:hover': {
        ...
        boxShadow: `0 15px 40px ${modernColors.primary.main}40`,
...
const ModernStepIconRoot = styled('div')(({ theme, ownerState }) => ({
    ...
    justifyContent: 'center',
    borderRadius: '50%',
    border: `3px solid ${modernColors.neutral.lightGray}`,
    background: 'white',
    position: 'relative',
    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
...
const ModernFormControl = styled(FormControl)(({ theme }) => ({
    ...
    '& .MuiOutlinedInput-root': {
        ...
        '& fieldset': {
            border: `2px solid ${modernColors.neutral.lightGray}30`,
            transition: 'all 0.3s ease',
        },

        '&:hover fieldset': {
            border: `2px solid ${modernColors.primary.main}50`,
        },

        '&.Mui-focused fieldset': {
            border: `2px solid ${modernColors.primary.main}`,
            boxShadow: `0 0 0 4px ${modernColors.primary.main}15`,
        },
    },
...
const ModernMenuItem = styled(MenuItem)(({ theme }) => ({
    ...
    '&:hover': {
        background: `${modernColors.primary.main}10`,
        color: modernColors.primary.main,
    },

    '&.Mui-selected': {
        background: `${modernColors.primary.main}15`,
        color: modernColors.primary.main,
        fontWeight: 600,
...
const ModernFilterButton = styled(Button)(({ isActive }) => ({
    background: isActive ? modernColors.primary.gradient : 'rgba(255, 255, 255, 0.95)',
    color: isActive ? 'white' : modernColors.primary.main,
    fontWeight: 'bold',
    fontSize: '0.875rem',
    padding: '12px 24px',
    borderRadius: '16px',
    border: `2px solid ${isActive ? 'transparent' : modernColors.primary.main}30`,
    margin: '0 8px 8px 0',
    textTransform: 'none',
    backdropFilter: 'blur(10px)',
    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
    boxShadow: isActive
        ? `0 8px 25px ${modernColors.primary.main}30`
        : '0 4px 15px rgba(0, 0, 0, 0.1)',

    '&:hover': {
        background: isActive
            ? modernColors.primary.gradient
            : `${modernColors.primary.main}10`,
        transform: 'translateY(-2px)',
        boxShadow: isActive
            ? `0 12px 35px ${modernColors.primary.main}40`
...
export default function FoundationTestSeries({
    cartNumberUpdate
}) {
    // Inject modern CSS animations
    React.useEffect(() => {
        const style = document.createElement('style');
        style.textContent = modernAnimations;
    ...
    function handleNextBrowse() {
        ...
        router.push('/foundation-test-series?data=' + Base64.encode(JSON.stringify(data), true));
    }
            ...
            if (selectBtnType === 'both') {
                ...
                if (plan.children?.length > 0) {
                    plan.children.forEach((group) => {
                        ...
                        if (!checkSubjectExists(subjectTempList, group.title)) {
                            subjectTempList.push(group);
                        }
                        // })
                        // }
                        // }
    ...
    function getPlanPrice(plan, selectedGroup, sltSubject) {
        ...
        if (selectedGroup === 'both') {
            ...
            if (Array.isArray(plan?.children)) {
                totalSubject = plan.children.length;
            }
            ...
            if (selectedSubject.length === totalSubject || selectedSubject.length === 0) {
                ...
                // Loop through each subject in plan.children
                plan?.children?.forEach((subject) => {
                    const isSelected = selectedSubject?.find(
                        (s) => s.entityId === subject.entityId
                    );

                    if (isSelected) {
                        selectedOfGroup.push(isSelected);
                        groupSelectedSubject += 1;
                    }
                });

                // If all children are selected
                if (groupSelectedSubject === plan?.children?.length) {
                ...
                if (allSubjectSelectOfGroup && groupSelectedSubject > 0) {
                    ...
                    selectedOfGroup.forEach((selectedSubject) => {
                        ...
                        thumbLogo = selectedSubject?.description?.thumb;
                ...
                <DialogContent style={{ textAlign: "center" }}>
                    {
                        selectedBasicPlan?.title === "Cumulative" ?
                            <img
                                src="img/cumulative.jpg"
                                alt="Scheduled"
                                style={{ margin: "5px" }}
                            /> : selectedBasicPlan?.title === "Exclusive" ?
                                <img
                                    src="img/exclusive.jpg"
                                    alt="Scheduled"
                                    style={{ margin: "5px" }}
                                /> : ""
                    }
